...
class ScriptRepository(BaseRepository):
    ...
    def get_all(self, tag: Optional[str] = None, search: Optional[str] = None) -> List[Script]:
...
class ExecutionRepository(BaseRepository):
    ...
    def get_all(self, script_id: Optional[str] = None, limit: int = 100) -> List[Execution]:
    ...
    def count_by_status(self, status: str) -> int:
        ...
        return self.session.query(func.count(Execution.id))\
            .filter(Execution.status == status).scalar()
